...
for (const day of days) {
    ...
    console.log(`Vào ${day}, chúng tôi mở vào ngày ${day}`) ?? 'closed';
}

// method
console.log(restaurant.order?.(0,1) ?? 'phương thức bạn yêu cầu k xác định');
console.log(restaurant.orderPizza?.(0,1) ?? 'phương thức bạn yêu cầu k xác định');

// array

const users = [{name: 'AnLaoGia', email: 'Anpnb79@lmao.com'}]

console.log(users[0]?.name ?? 'user của bạn chưa có tên')

if(users.length > 0) {
    console.log(users[0].name)
} else {
    console.log('người dùng chưa có tên')
}
